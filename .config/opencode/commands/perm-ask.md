---
description: "Persistently set a granular permission rule to ask"
---

Add a permanent OpenCode permission rule with action `ask`.

Usage:
- `/perm-ask <tool> <pattern>`
- `/perm-ask bash "git commit *"`
...
3. In JSON:
   ...
   - Upsert rule so `<tool>` includes `<pattern>: "ask"`.
